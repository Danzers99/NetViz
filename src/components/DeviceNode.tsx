...
import { DRAG_BOUND_HALF_SIZE } from '../utils/constants';
    ...
    const { handlePointerDown, handlePointerUp, handlePointerMove } = useDraggable({
        ...
        onDrag: (snappedDelta) => {
            ...
            Object.entries(initialPositionsRef.current).forEach(([id, initPos]) => {
                ...
                updates[id] = [
                    Math.max(-DRAG_BOUND_HALF_SIZE, Math.min(DRAG_BOUND_HALF_SIZE, rawX)),
                    initPos[1],
                    Math.max(-DRAG_BOUND_HALF_SIZE, Math.min(DRAG_BOUND_HALF_SIZE, rawZ)),
                ];
            });
