...
export const WifiCoverageOverlay = ({ device }: WifiCoverageOverlayProps) => {
    ...
    // Animation Loop — wrapped in try/catch so errors disable overlay instead of crashing the scene
    useFrame(() => {
        ...
        try {
            ...
            const posAttr = geometryRef.current.getAttribute('position') as THREE.BufferAttribute | null;
            const colAttr = geometryRef.current.getAttribute('color') as THREE.BufferAttribute | null;

            // Guard: geometry not yet initialized by useEffect
            ...
            // Outline Update (guard against uninitialized geometry)
            if (outlineRef.current && outlineGeoRef.current && isDarkFrame) {
                const outPos = outlineGeoRef.current.getAttribute('position') as THREE.BufferAttribute | null;
                if (outPos) {
                    for (let i = 0; i <= SEGMENTS; i++) {
            ...
            // Sync outline material color with frame-accurate theme
            if (outlineRef.current) {
                const lineMat = (outlineRef.current as any).material as THREE.LineBasicMaterial;
                if (lineMat) {
                    const outlineColor = isDarkFrame ? new THREE.Color("#67e8f9") : new THREE.Color("#0891b2");
                    lineMat.color.copy(outlineColor);
                    lineMat.opacity = isDarkFrame ? 0.4 : 0;
                }
            }
        } catch (err) {
            // Graceful degradation: hide the overlay instead of crashing the scene
            console.warn('[WifiCoverageOverlay] Render error, disabling overlay for device:', device.id, err);
    ...
    return (
        <group position={[0, 0.02, 0]} rotation={[-Math.PI / 2, 0, 0]}>
            ...
            {/* Always mounted — visibility controlled in useFrame to avoid mount/unmount crash */}
            {/* @ts-ignore: R3F <line> JSX element */}
            <line position={[0, 0, 0.06]} ref={outlineRef} visible={false}>
