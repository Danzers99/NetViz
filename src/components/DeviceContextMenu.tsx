 ...
 *
 * Shared right-click action menu used by both 3D sandbox and 2D Network Diagram.
 * Device-specific actions (WiFi, Power) are controlled by device capabilities,
 * not by view context. Both views show identical menus for the same device.
 */
...
interface DeviceContextMenuProps {
    device: Device;
    onClose: () => void;
    /** View context — currently reserved for future view-specific logic */
    context: '3d' | '2d';
}

type MenuState = 'main' | 'wifi_connect' | 'wifi_host';

/* ── WiFi Connect Sub-menu ────────────────────────────────── */
        ...
        <div className="p-2 space-y-2">
            ...
            <div>
                ...
                <input
                    ...
                    value={ssid}
                    onChange={(e) => { setSsid(e.target.value); setStatus('idle'); }}
    ...
    const handleSave = () => {
        updateDevice(device.id, {
            wifiHosting: {
                ...device.wifiHosting!,
                configs: [
                    { ...device.wifiHosting!.configs[0], ssid, password },
...
/* ── Main Shared Context Menu ────────────────────────────── */

export const DeviceContextMenu = ({ device, onClose, context: _context }: DeviceContextMenuProps) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-600 rounded shadow-xl flex flex-col min-w-[140px] overflow-hidden text-sm">
            ...
            {/* WiFi actions — shown only for WiFi-capable devices */}
            ...
            {/* Power actions — CakePop/OrderPad only */}
